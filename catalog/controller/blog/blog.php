...
//			$limit = $this->config->get('ocblog_article_limit');
		...
		if(isset($this->request->get['list_id'])) {
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link('blog/blog', '&list_id=' . $this->request->get['list_id'])
            );
        }
		...
		if(isset($this->request->get['list_id'])) {
			$article_list_id = $this->request->get['list_id'];
		} else
			$article_list_id = 44;
        ...
        if($results){
            ...
            foreach ($results as $result) {
                $data['articles'][] = array(
                    ...
                    'image'		  => $this->model_tool_image->resize($result['image'], 135, 135),
            ...
            $data['article_list'] = $this->model_blog_article->getArticlesList($article_list_id);

            if($data['article_list']['meta_title'] != '')
                $this->document->setTitle($data['article_list']['meta_title']);
            else
                $this->document->setTitle($data['article_list']['name']);

            $this->document->setDescription($data['article_list']['meta_description']);
            $this->document->setKeywords($data['article_list']['meta_keyword']);
