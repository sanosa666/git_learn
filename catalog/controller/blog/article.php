...
class ControllerBlogArticle extends Controller {
	public function index() {
		...
		if ($article_info) {
			...
			$this->document->setOgImage($this->model_tool_image->resize($article_info['image'], 1200, 630));
            ...
            $article_list_info = $this->model_blog_article->getArticleListDescription($article_id);
//            var_dump($article_list_info);
            $data['article_list']['href'] = $this->url->link('blog/blog', '&list_id=' . $article_list_info['article_list_id']);
            $data['article_list']['name'] = $article_list_info['name'];
